...
def get_output_filename():
    """Генерирует имя файла. Если задана ZAKUPKI_OUTPUT_DIR - пишет туда."""
    now = datetime.now()
    ...
    if session_dir and os.path.isdir(session_dir):
        # Режим run_all.py: пишем в указанную папку
        folder_path = session_dir
        print(f"📁 Используем папку сессии: {folder_path}")
    else:
        # Режим отдельного запуска: создаём свою папку
